...
def run_auto_split_analysis(data_file):
    ...
    if n_plots > 0:
        ...
        # fig1.suptitle('Figure 1: Validation Parity Plots (Random 10% Verify Split)', fontsize=16)
        ...
        for i, (key, label) in enumerate(targets):
            ...
            # 첫 번째 그래프(Q'')에 대해 x10^3 스케일 및 간격 설정
            if i == 0 and key == 'q_flux':
                ...
                # ax.set_xlabel(f"Actual {label} (×10³)", fontsize=18)
                # ax.set_ylabel(f"Predicted {label} (×10³)", fontsize=18)
    ...
    if n_plots > 0:
        ...
        # fig2.suptitle('Figure 2: Feature Importance', fontsize=16)
        ...
        for i, (key, label) in enumerate(targets):
            ...
            print(f"\n{label}:")
            for name, value in zip(feat_names, sens):
                print(f"  {name}: {value:.2f}%")
            
            sns.barplot(x=feat_names, y=sens, ax=axes2[i], palette='viridis')
            # axes2[i].set_title(f"{label} Sensitivity")
            axes2[i].set_ylabel("Importance (%)", fontsize=15)
            axes2[i].set_xlabel("", fontsize=15)
            axes2[i].tick_params(axis='both', labelsize=15)
    ...
    # fig.suptitle('Figure 3: 3D Response Surfaces', fontsize=16)
    ...
    for key, label in targets:
        for i, (idx1, idx2, name1, name2) in enumerate(pairs):
            ...
            # Q'' 그래프는 z축 라벨을 그래프에서 더 띄워서 표시
            if key == 'q_flux':
                ax.set_zlabel(label, fontsize=12, labelpad=15)
            else:
                ax.set_zlabel(label, fontsize=12)
            
            ax.tick_params(axis='both', labelsize=12)
            
            # Q'' 그래프에 대해 z축 간격 25000으로 설정 (100000, 125000, 150000이 표시되도록)
            if key == 'q_flux':
                from matplotlib.ticker import MultipleLocator
                ax.zaxis.set_major_locator(MultipleLocator(25000))
