...
# =============================================================================
# 3) New friction factor correlation + ΔP
# =============================================================================

def nir_friction_factor(Re_Dc: float, s1_Dc: float, area_ratio: float) -> float:
    """
    Fitted Correlation(Not Nir)
        f_N = 1.1 * Re^(-0.25) * (S1/Dc)^(0.6) * (A_tot/A_bare)^(0.15)
    """
...
def main():
    ...
    ap.add_argument("--in", dest="in_csv", required=True,
                    help="Input design CSV filename or path (default: HeatExchanger/data/<name>)")
    ap.add_argument("--out", dest="out_csv", default="porous_from_design.csv",
                    help="Output CSV filename or path (default: HeatExchanger/data/<name>)")


    ap.add_argument("--T", type=float, default=14.8, help="Ambient temperature [°C]")
    ap.add_argument("--v", type=float, default=2.019, help="Design inlet velocity [m/s]")

    ap.add_argument("--Dc", type=float, default=24.0, help="Tube OD [mm]")
    ap.add_argument("--delta_f", type=float, default=0.5, help="Fin thickness [mm]")
    ap.add_argument("--pitch_ratio", type=float, default=1.0, help="s1/s2 ratio")
    ap.add_argument("--N", type=int, default=4, help="Number of tube rows N")

    ap.add_argument("--v_min", type=float, default=0.5, help="Fitting velocity min [m/s]")
    ap.add_argument("--v_max", type=float, default=3.5, help="Fitting velocity max [m/s]")
    ap.add_argument("--n_points", type=int, default=50, help="Number of fitting points")

    ap.add_argument("--max_rows", type=int, default=None, help="Process only first K rows (debug/speed test)")
    ap.add_argument("--progress_every", type=int, default=500, help="Progress print interval")

    ap.add_argument("--check_constraint", action="store_true",
                    help="Check fh <= fh_upper(s1) and record violations (does not drop rows)")
    args = ap.parse_args()
    # === Resolve project paths relative to THIS script ===
    script_path = Path(__file__).resolve()  # .../HeatExchanger/scripts/porous_calc/porous_converter.py
    project_root = script_path.parents[2]   # .../HeatExchanger
    data_dir = project_root / "data"
    # Input: if user passed only a filename, assume data/
    in_path = Path(args.in_csv)
    if not in_path.is_absolute() and in_path.parent == Path("."):
        in_path = data_dir / in_path
    in_path = in_path.resolve()

    # Output: if user passed only a filename, assume data/
